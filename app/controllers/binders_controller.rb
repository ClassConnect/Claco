class BindersController < ApplicationController
	...
	def create
		...
		if params[:foldertitle].strip.length > 0
			...
			respond_to do |format|
				format.html {render :text => errors.empty? ? (@parent == "0" ? {"success" => 1, "data" => named_binder_route(new_binder)}.to_json : 1) : errors.map{|err| "<li>#{err}</li>"}.html_safe}
			end
	...
	def createcontent
		...
		if params[:webtitle].strip.length > 0
			...
			respond_to do |format|
				format.html {render :text => errors.empty? ? 1 : errors.map{|err| "<li>#{err}</li>"}.html_safe}
			end
